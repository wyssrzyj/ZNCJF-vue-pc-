 ...
 * 按传入的字段，过滤树结构数据
 * @param treeList 原始的树
 * @param paramsKey 要过滤的属性
 * @param paramsValue 要过滤的属性值
 * @param childField 子节点属性
 */
export const filterTreeByParams = (treeList = [], paramsKey = 'type', paramsValue = '0', childField? = 'children') => {
  return treeList.filter(item => {
    item[childField] = filterTreeByParams(item[childField], paramsKey, paramsValue, childField)
    return item[paramsKey] == paramsValue
  })
}
