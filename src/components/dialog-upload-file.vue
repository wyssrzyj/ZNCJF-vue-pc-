  ...
  <el-dialog v-model="state.dialogVisible" :title="state.title" :close-on-click-modal="false" draggable>
    <el-form>
      ...
      <el-form-item label="请选择需要上传的文件类型：">
        <el-select v-model="state.fileType" placeholder="请选择" :disabled="state.uploadFileLoading">
          <el-option v-for="item in state.fileTypeList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="批量导入：">
        <el-upload
          v-if="state.fileType !== null"
          :file-list="state.targetArr[state.fileType]['fileList']"
        ...
        >
          ...
          <div v-if="state.fileType !== null" class="subtip">
            ...
            文件命名格式：款号_阶段_序号，如A11M32801F_大货_01
          </div>
        </el-upload>
    ...
    <template #footer>
      <el-button :disabled="state.uploadFileLoading" @click="handleCancel">{{ $t('cancel') }}</el-button>
      <el-button type="primary" :disabled="state.uploadFileLoading" @click="handleSubmit">
        {{ $t('confirm') }}
      </el-button>
    </template>
  </el-dialog>
