  ...
  <el-dialog v-model="state.dialogVisible" :title="state.title" :close-on-click-modal="false" draggable>
    <el-form>
      ...
      <el-form-item label="批量导入：">
        <div class="flex">
          <njp-download-template v-if="state.fileType == 2" template-name="尺寸表模板.xlsx" />
          <el-upload
...
<script lang="ts" setup>
  ...
  const handleTypeChange = e => {
    if (e == 2) {
      state.ossAction = `${proxy.$baseService.app.api}/njp-dsr-api/dsr/dsrdataversionsize/importData`
    } else {
  ...
  const beforeAvatarUpload = rawFile => {
    ...
    if (!reg.test(rawFile.name)) {
      ElMessage.warning('请上传按“版型编码_自定义名称”规则命名的文件!')
      return false
    }
