 ...
 * @LastEditTime: 2022-08-26 15:39:13
 * @Description: 
 * @LastEditors: lyj
-->
<template>
    ...
    <el-table :header-cell-style="{ 'text-align': 'center' }" :cell-style="{ 'text-align': 'center' }" :data="state.tableData" border>
      ...
      <el-table-column v-for="(item, i) in state.data" :key="i" :label="item.name" width="60">
        <template #default="scope">
          <el-input-number v-model="scope.row[item.model]" class="fabricLayingInput" :controls="false" :precision="1" :step="0.1" @change="handleChange" />
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script lang="ts" setup>
  ...
  import { tableMiddleData, tableData } from './conifgs.ts'
  import './index.css'
