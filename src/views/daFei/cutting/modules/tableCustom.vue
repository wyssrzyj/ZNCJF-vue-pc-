 ...
 * @LastEditTime: 2023-04-16 17:14:45
 * @Description: 
 * @LastEditors: lyj
-->
<template>
                ...
                <tr v-for="item,index in state.list" :key="index" class="tableDraggable-body handle">
                  ...
                  <td class="common">
                    <div class="input-common">
                      <el-input @change="(e:any)=>change(item,2)" v-model="item.vatNo" placeholder="缸号" />
                    </div>
                  </td>
                  <td class="common">
                    <div class="input-common">
                      <el-input @change="(e:any)=>change(item,2)" v-model="item.clothNo" placeholder="匹号" />
                    </div>
                  </td>
                  <td class="common-two">
                    <div class="input-demo">
                      <el-input-number :controls="false" @change="(e:any)=>change(item,3)" v-model="item.clothNoStart" placeholder="开始位置" />
                      <el-input-number :controls="false" @change="(e:any)=>change(item,3)" v-model="item.clothNoEnd" placeholder="结束位置" />
                    </div>
                  </td>
                  <td class="common">
                    <div class="input-common">
                       <el-input-number :controls="false" v-model="item.spreadClothLevel" :min="1"    placeholder="层数" @change="(e:any)=>change(item,1)"/>
                    </div>
                  </td>
                  <!-- 动态尺码 -->
                  <td v-for="v in state.size" :key="v" class="common-two">
                    <div class="input-demo-top">
                      <!-- <div class="input-demo-tiehao">
                       <el-input-number :controls="false" v-model="item[v.size].tiehao" :min="1"    placeholder="扎号" @change="(e:any)=>change(item,2)"/>
                      </div> -->
                      <div class="input-demo-size">
                       <!-- <el-input-number :controls="false" v-model="item[v.size].scale" :min="1"  @change="(e:any)=>change(item,1)"   placeholder="比例" /> -->
                       <el-input-number :controls="false" v-model="item[v.size].tiehao" :min="1"    placeholder="扎号" @change="(e:any)=>change(item,2)"/>
                       <el-input-number  :controls="false" v-model="item[v.size].quantity" :min="1"  dis   placeholder="件数"/>
                      </div>
                  </div>
                  </td>
                </tr>
      ...
      if (isEmpty(list)) {
        if (isEmpty(state.size)) {
          ...
          only: `${e.color}${list.length + 1}`,
           fabricColor: e.color, 
           vatNo: '', clothNo: '', clothNoStart: '', clothNoEnd: '', spreadClothLevel: '' }
          list.push(data)
          state.list = list
        } else {
          ElMessage({
